...
compareLengthFirst a b = case comparing length a b of
  EQ -> comparing head b a
  x -> x

longest :: Ord a => [[a]] -> [a]
longest = maximumBy compareLengthFirst
...
incrementKey key = map (\x -> (fst x, snd x + if fst x == key then 1 else 0))
...
winner scores player1 player2 = maximumBy (comparing $ \name -> Map.findWithDefault 0 name scores) [player2, player1]

-- Kirottu ratkaisu, joka taitaa olla implementaatiospesifi ja hajoaa (minun koneella) jos player1 ja player2 vaihtavat paikkaa.
...
freqs = foldr (Map.alter incMaybe) Map.empty
...
transfer from to amount bank
  | maybe True (< amount) (Map.lookup from bank) || Map.notMember to bank || amount < 0 = bank
  | otherwise = Map.adjust (\sum -> sum - amount) from $ Map.adjust (+ amount) to bank
