...
firstHalf s = (take . ((`div` 2) . (+ 1) . length)) s s
...
joinToLength n l = filter ((== n) . length) [x ++ y | x <- l, y <- l]
...
interpreter = simulate 0 0 []

simulate :: Int -> Int -> [String] -> [String] -> [String]
simulate _ _ outs [] = outs
simulate x y outs commands = case head commands of
  "up" -> simulate x (y + 1) outs (tail commands)
  "down" -> simulate x (y - 1) outs (tail commands)
  "left" -> simulate (x - 1) y outs (tail commands)
  "right" -> simulate (x + 1) y outs (tail commands)
  "printX" -> simulate x y (outs ++ [show x]) (tail commands)
  "printY" -> simulate x y (outs ++ [show y]) (tail commands)
  _ -> outs
